...
if [ "$AKASH_STATESYNC_ENABLE" == true ]; then
  ...
  export AKASH_STATESYNC_TRUST_HASH=$TRUST_HASH
    ...
    SNAPSHOT_URL=$(curl https://polkachu.com/tendermint_snapshots/akash | grep tar.lz4 | head -n1 | grep -io '<a href=['"'"'"][^"'"'"']*['"'"'"]' |   sed -e 's/^<a href=["'"'"']//i' -e 's/["'"'"']$//i')
    ...
    lz4 -c -d snapshot.tar.lz4 | tar -x -C $AKASH_HOME/data
